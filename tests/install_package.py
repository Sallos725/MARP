...
for arch,machine in [("amd64",62),("arm64",183)]:
    data=(dist/f"marp-v0.9.0-linux-{arch}").read_bytes()
...
with tempfile.TemporaryDirectory(prefix="marp-install-") as tmp:
    root=Path(tmp)
    with zipfile.ZipFile(dist/"multiagent-full-v0.9.0.zip") as z:
        assert z.testzip() is None
        assert all(not Path(n).is_absolute() and ".." not in Path(n).parts for n in z.namelist())
        z.extractall(root)
    assert (root/"plugin/multiagent-full-v0.9.0.js").exists()
    assert not (root/"app").exists()
    for arch in ["amd64","arm64"]:
        assert (root/f"bin/marp-v0.9.0-linux-{arch}").read_bytes()==(dist/f"marp-v0.9.0-linux-{arch}").read_bytes()
    ...
    try:
        ...
        assert request("/status")["version"]=="0.9.0"
        ...
        binary=root/"bin/marp-v0.9.0-linux-amd64"
        assert subprocess.check_output([str(binary),"--version"],env=env,text=True).strip()=="0.9.0"
