...
for arch,machine in [("amd64",62),("arm64",183)]:
    data=(dist/f"marp-v0.9.5-linux-{arch}").read_bytes()
...
with tempfile.TemporaryDirectory(prefix="marp-install-") as tmp:
    root=Path(tmp)
    with zipfile.ZipFile(dist/"multiagent-full-v0.9.5.zip") as z:
        assert z.testzip() is None
        assert all(not Path(n).is_absolute() and ".." not in Path(n).parts for n in z.namelist())
        z.extractall(root)
    assert (root/"plugin/multiagent-full-v0.9.5.js").exists()
    assert not (root/"app").exists()
    for name in ["DIAGNOSTICS", "SETTINGS", "PDF", "DEVELOPMENT"]:
        assert (root/f"guides/{name}.md").exists(), name
    for arch in ["amd64","arm64"]:
        assert (root/f"bin/marp-v0.9.5-linux-{arch}").read_bytes()==(dist/f"marp-v0.9.5-linux-{arch}").read_bytes()
    ...
    try:
        ...
        assert request("/status")["version"]=="0.9.5"
        ...
        binary=root/"bin/marp-v0.9.5-linux-amd64"
        assert subprocess.check_output([str(binary),"--version"],env=env,text=True).strip()=="0.9.5"
