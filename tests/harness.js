...
window.setup=async full=>{
 ...
 nativeFetch:async(url,req={})=>{
  ...
  else if(path==='/analyze'){window.lastPayload=JSON.parse(req.body);data={context_world:'World fact',context_plot:'Plot note',context_char:'Character motive',errors:{}}}
 ...
 window.host=host;window.instance=await start(host,{full,analyze,defaultPrompts});window.run=(m,mode='model')=>before(m,mode);window.unload=()=>unload();
};
