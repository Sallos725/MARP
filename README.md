...
- **프롬프트+모델+온도 프리셋**: Lite/Full 설정 GUI에서 현재 프롬프트 override와 모델/온도 조합을 compact JSON으로 export/import할 수 있다. 기본 프롬프트는 내장 rollback 기준으로 유지하고, 프리셋에는 사용자 override만 담아 모바일 부담을 줄인다.
- **RisuAI 업데이트 체크**: `//@update-url`과 `//@link` 메타데이터를 추가해 RisuAI 플러그인 목록에서 새 버전이 감지되면 업데이트 버튼으로 갱신하고 GitHub 링크로 이동할 수 있다.
