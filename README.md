...
**v0.9.7에서는 설정 화면을 심해 네이비 + 골드 테마로 바꾸고, 분석 진행과 결과를 채팅 화면 오른쪽 위에 작게 띄우는 진행 표시를 추가했습니다.**

[릴리즈 노트](https://github.com/Sallos725/MARP/releases/tag/v0.9.7) · [진단 화면 사용법](guides/DIAGNOSTICS.md) · [전체 변경 내역](RELEASE_NOTES.md)

## 다운로드

| 사용 방식 | 받을 파일 | 실행 위치 |
| --- | --- | --- |
| Lite | [multiagent-lite-v0.9.7.js](https://github.com/Sallos725/MARP/releases/download/v0.9.7/multiagent-lite-v0.9.7.js) | RisuAI 브라우저 플러그인 |
| Full | [multiagent-full-v0.9.7.zip](https://github.com/Sallos725/MARP/releases/download/v0.9.7/multiagent-full-v0.9.7.zip) | RisuAI 브라우저 플러그인 + 별도 서버 |

Full ZIP에는 플러그인, Docker 구성, Linux amd64·arm64 실행 파일과 사용 문서가 들어 있습니다. 실행 파일만 필요하면 [릴리즈 첨부 파일](https://github.com/Sallos725/MARP/releases/tag/v0.9.7)에서 해당 아키텍처와 `SHA256SUMS`를 받습니다.
...
ZIP의 `plugin/multiagent-full-v0.9.7.js`를 RisuAI에 가져오고 MARP **공통** 탭에서 서버 URL을 지정합니다. 기본 포트는 `6009`입니다. 휴대전화에서는 휴대전화의 localhost가 아닌 서버에 접근할 수 있는 주소를 입력합니다.
...
플러그인 ID와 자동 업데이트 주소, Docker 이미지 이름·서비스 이름·데이터 경로는 유지됩니다. v0.8.x 프리셋도 가져올 수 있습니다. Full 구버전 서버는 기존 응답으로 동작하며, 단계 시작 시각의 실측은 Full 서버 v0.9.1 이상에서 지원합니다.

## UI 열기

채팅 입력창 왼쪽의 햄버거 메뉴에서 **MARP Lite** 또는 **MARP Full**을 선택합니다. 캐릭터 목록 쪽 햄버거 메뉴에는 MARP 항목을 표시하지 않습니다. 플러그인 설정 화면에서도 같은 UI를 열 수 있습니다.

v0.9.1 Full 서버를 사용 중이라면 이번 메뉴 이동은 브라우저 플러그인만 v0.9.2로 업데이트해도 적용됩니다. 서버 분석 동작은 동일합니다.

## 채팅 화면 진행 표시

공통 탭의 **채팅 화면 진행 표시**를 켜면 분석 진행과 결과가 채팅 화면 오른쪽 위에 작게 뜹니다. 기본값은 꺼짐입니다.

- 처음 켤 때 RisuAI가 "메인 Document 접근" 권한을 묻습니다. MARP는 이 권한으로 표시 하나를 그리고, 겹치지 않도록 NMOS 표시의 위치만 읽습니다.
- 점 세 개는 세계관 · 플롯 · 등장인물입니다. 골드 = 진행 중, 초록 = 성공, 빨강 = 실패, 회색 = OFF.
- Lite는 단계별로 갱신되고, Full은 진행 중 경과 시간만 보이다가 끝나면 결과를 표시합니다. 결과는 4초 뒤 사라집니다.
- NMOS 진행 표시가 오른쪽 위에 있으면 그 아래에 붙습니다. RisuAI 요청 상태 알림이 떠 있는 동안에는 가려질 수 있습니다.
- 표시를 누르면 MARP 설정이 열립니다.
- 권한을 거부했다면 설정 → 플러그인 → MARP 줄 메뉴 → **권한 응답 초기화** 후 다시 켜세요.
