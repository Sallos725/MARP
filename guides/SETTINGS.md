...
- 실행된 분석이 오류 없이 끝난 요청은 현재 플러그인 세션의 메모리에 최대 10분·최근 8건까지 보관합니다. 메인 생성이 실패해 같은 입력을 재전송하면 저장된 분석을 다시 주입하므로 보조 LLM을 다시 호출하지 않습니다. 동시에 들어온 동일 요청도 하나의 분석 작업을 공유합니다.
