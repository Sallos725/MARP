...
import promptPack from './prompts.json';
...
export async function analyze(host,c,input,signal){
 ...
 const run=async name=>{
  ...
  const timer=deadline(signal,c.request_timeout*1000),start=performance.now();
  try{const r=await guarded(callAgent(host,resolve(c,name),prompts(name,c,values),timer.signal),timer.signal);result.diagnostics[name]={status:'success',usage:r.usage,pdf:r.pdf,chars:r.text.length};return r.text}
  catch(e){result.errors[name]=e.message;result.diagnostics[name]={status:'error',error:e.message};return ''}
  finally{timer.close();result.latency_ms[name]=Math.round(performance.now()-start)}
 };
