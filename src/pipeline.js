...
export async function analyze(host,c,input,signal,onProgress){
 const started=performance.now(),report=(name,status)=>{try{onProgress?.(name,status)}catch{}};
 ...
 const run=async name=>{
  ...
  try{report(name,'running');const r=await guarded(callAgent(host,a,prompts(name,c,values),timer.signal),timer.signal);result.diagnostics[name]={status:'success',usage:r.usage,pdf:r.pdf,chars:r.text.length};report(name,'success');return r.text}
  catch(e){result.errors[name]=e.message;result.diagnostics[name]={status:'error',error:e.message};report(name,'error');return ''}
